...
**达人端**：任务广场（筛选/排序/搜索/AI 推荐分/名额进度/已抢光）、任务详情（结算规则与资金保障）、我的任务（接单→提交→审核→托管划付）、多平台一键发布（实时预览/配图/定时）、互动大厅（冷却+限额）、AI 打卡（日历+七日递进）、收益钱包（充值/提现/明细/7 日图表/等级/收益计算器）、龙虎榜（真实收益排行）。

**商家端**：数据看板（托管资金/消耗趋势）、三步投放向导（余额不足引导充值）、任务管理（托管预算/已消耗/暂停恢复）。

**通用**：达人端/商家端切换、明暗双主题、两行式导航（全栏目始终可见）、移动端响应式、登录注册弹窗、邀请码、Toast/抽屉/模态体系。
